...
> = {
	...
	givens: {
		...
		WidthAndHeightOf: (width: number, height: number) =>
			new Rectangle(width, height),
	},

	whens: {
		HeightIsPubliclySetTo:
			(height: number) => async (rectangle: Rectangle, utils: PM) => {
				rectangle.setHeight(height);
				return rectangle;
			},
		WidthIsPubliclySetTo:
			(width: number) => async (rectangle: Rectangle, utils: PM) => {
