...
const transformJpegs = (jpgs: any[], assets: any[], blogEntries: any[]) => {
  return jpgs.reduce(
    (
      mm: any,
      jpg: { src: any; content: string | ArrayBuffer | Buffer<ArrayBufferLike> }
    ) => {
      const src = jpg.src;
      const jpgSplit = src.split("/");

      const asset = assets.find((asset: { src: string }) => {
        return (
          src.split("/").slice(0, -1).join("") ===
          asset.src.split("/").slice(0, -1).join("")
        );
      });
      ...
      if (asset) {
        ...
        const modifiedJpgs = Object.keys(transformations).reduce(
          (mmm, transformationKey) => {
            const transformation = transformations[transformationKey];

            const modifedImagePromise = new Promise(async (res, rej) => {
              const image = await Jimp.read(jpg.content);

              // const batchImage = image.batch();
              transformation.forEach((transform) => {

                // image.apply(transform[0], transform[1])
                // image.apply(transform[0], transform[1])
              ...
              res(image);
              // image.bitmap.data.toBuffer("jpg", {}, (err, buffer) => {
              //   res(buffer);
              // });
            ...
            const blogFolder = blogEntries.find((b: { srcFolder: any }) =>
            ...
            return {
              ...
              transformationKey +
              "-" +
              jpgSplit[jpgSplit.length - 1]]: modifedImagePromise,
            };
          },
          {}
        );
...
const processBlogEntries = (blogEntries: any[]) => {
  ...
  const processedEntries = blogEntries
    ...
    .filter(
      (a: { meta: { published: boolean } }) => a.meta.published !== false
    );
  ...
  outputs: (_: {
    ...
    const $blogEntriesJpgsModified = $$$(
      [
        ...
        $$$([srcAndContentOfFiles(_[BLOG_ASSETS])], (assets) =>
          assets.map((asset: { src: any; content: string }) => {
            return {
              src: asset.src,
